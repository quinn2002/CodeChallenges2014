...
{
	public class CodeChallenge04_MathExpressions : ICodeChallenge
	{
		public string OutputResult(OrderedDictionary inputValues)
		{
			string userInput = inputValues[0].ToString();

			userInput = userInput.RemoveAllSpaces();
			if (userInput.IsValidMathmaticalExpression())
			{
				return String.Format("{0} = {1}", userInput, userInput.EvaluateMathmaticalExpression());
			}
	...
	public static class MathmaticalExpressionExtensions
	{
		...
		public static string EvaluateMathmaticalExpression(this string input)
		{
			...
			currentExpressionStr.EvaluateExpressionNoParens(expression);
