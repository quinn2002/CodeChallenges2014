...
{
    public class CodeChallenge08_LoremIpsumGenerator : ICodeChallenge
    {
        public string OutputResult(OrderedDictionary inputValues)
        {
            ...
            string userOption = inputValues[1].ToString();
            int userOptionNum;

            if (Int32.TryParse(userOption, out userOptionNum) && classDictionary.ContainsKey(userOptionNum))
            {
                string userWordCountStr = inputValues[0].ToString();
                Int16 userWordCount;
                if (Int16.TryParse(userWordCountStr, out userWordCount) && userWordCount > 0)
                {
                    var selectedClass = classDictionary[userOptionNum];
                    return selectedClass.GenerateText(userWordCount, selectedClass.TxtFileName);
                }
                else
                {
                    return String.Format("Invalid number.  Make sure to pick a valid number of words to generate (< {0}).", Int16.MaxValue);
                }
            }
            else
            {
                return String.Format("Invalid inputs.  Please try again.");
            }
        }
    ...
    public abstract class LoremIpsumBase
    {
        ...
        public string GenerateText(int numWords, string fileName)
        {
            ...
            List<string> wordList = FileReader.GetWordsFromFile(AppDomain.CurrentDomain.BaseDirectory + @directoryPath + fileName);
