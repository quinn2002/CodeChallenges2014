...
{
    public class CodeChallenge06_CaesarCipher : ICodeChallenge
    {
        public string OutputResult(OrderedDictionary inputValues)
        {
            string userInput = inputValues[0].ToString();
            Int16 shiftPositionNum = 0;
            string shiftPosition = inputValues[1].ToString();

            if (Int16.TryParse(shiftPosition, out shiftPositionNum) && shiftPositionNum >= 0)
            {
                var abcInfo = new AlphabetInfo("eng", 'a', 'A', 'z', 'Z', '0', '9', 26, 10);
                string encrypted = EncryptCaesarCipher(userInput, abcInfo, shiftPositionNum);
                string decrypted = DecryptCaesarCipher(userInput, encrypted, abcInfo, shiftPositionNum);
                return String.Format("Shift position is: {0}" + 
                                        "\nEncrypted: {1}" + 
                                        "\nDecryption Check: {2}", shiftPosition, encrypted, decrypted);
            }
            else
            {
                return String.Format("Invalid shift position.  Make sure number is between {0} and {1}", 0, Int16.MaxValue);
            }
        }
