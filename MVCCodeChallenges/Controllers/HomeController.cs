...
using MVCCodeChallenges.ViewModels;
...
namespace MVCCodeChallenges.Controllers
{
	public class HomeController : Controller
	{
		...
		{
			var selectedYear = String.IsNullOrWhiteSpace(year) ? "2014" : year;
			var challenges = _db.Challenges
										.Include("ChallengeInputs")
										.Include("ChallengeInputs.InputLookupValues")
										.OrderBy(c => c.Sequence)
										.ToList();
			var years = challenges.Select(c => c.Year).Distinct();
			challenges = challenges.Where(c => c.Year == selectedYear).ToList();
			...
			if (Request.IsAjaxRequest())
			{
				...
				Int32.TryParse(challengeId, out challengeIdNum);
				string challengeClassPath = ChallengeClassPathGenerator.GetChallengeClassPath(challengeIdNum, challenges);
